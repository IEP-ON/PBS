...
export function AlertsView() {
  ...
  useEffect(() => {
    ...
    const loadAlerts = async () => {
      const url = `/api/extinction-alerts${showResolved ? '?showResolved=true' : ''}`
      const res = await fetch(url)
      const data = await res.json()
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🚨 소거 위험 경보</h1>
          <p className="text-xs text-gray-400 mt-0.5">강화 중단 후 일시적 행동 증가 패턴 · Lerman·Iwata(1995) 참고</p>
        </div>
      ...
      <div>
        <h2 className="font-bold text-gray-900 mb-3">🔔 활성 알림 ({activeAlerts.length}건)</h2>
        {activeAlerts.length === 0 ? (
      ...
      {showResolved && resolvedAlerts.length > 0 && (
        <div>
          <h2 className="font-bold text-gray-900 mb-3">✅ 해결된 알림 ({resolvedAlerts.length}건)</h2>
