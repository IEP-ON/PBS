...
export function FbaClassView() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage(`✅ 행동 원인 분석 기록 완료${data.aiAnalysisPerformed ? ' (AI 분석 포함)' : ''}`)
        setSelectedStudent('')
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">🔍 행동 원인 분석</h1>
        <p className="text-xs text-gray-400 mt-0.5">행동의 기능(원인)을 추정하는 사정 · Cooper 등(2020) ABA 개론 참고</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
        <h2 className="font-bold text-gray-900">새 사정 기록</h2>
        <select 
      ...
      <div>
        <h2 className="font-bold text-gray-900 mb-3">분석 기록 ({filteredRecords.length}건)</h2>
        {filteredRecords.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
            <p className="text-gray-400">등록된 분석 기록이 없습니다.</p>
              ...
              <div key={r.id} className={`bg-white rounded-2xl border border-l-4 p-5 ${
                ...
                {r.estimated_function && (
                  <div className="space-y-1.5 mb-2">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      <button
                        onClick={() => void fetchStrategies(r.estimated_function!)}
