...
export function DroManagementView() {
  const [students, setStudents] = useState<Student[]>([])
  const [goalOptionsByStudent, setGoalOptionsByStudent] = useState<Record<string, Goal[]>>({})
  ...
  useEffect(() => {
    if (!selectedStudent || goalOptionsByStudent[selectedStudent]) return

    let cancelled = false

    const loadGoals = async () => {
      const response = await fetch(`/api/pbs/goals?studentId=${selectedStudent}`)
      const data = await response.json()

      if (cancelled) return
      setGoalOptionsByStudent(prev => ({ ...prev, [selectedStudent]: data.goals || [] }))
    }
  ...
  }, [goalOptionsByStudent, selectedStudent])
  ...
  const goals = selectedStudent ? goalOptionsByStudent[selectedStudent] || [] : []
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">⏱️ 강화 타이머</h1>
