...
function InterventionsLibraryContent() {
  ...
  useEffect(() => { void fetchStrategies(filterFunction) }, [filterFunction])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        void fetchStrategies(filterFunction)
      } else {
      ...
      {fromFbaFunction && (
        <div className="flex items-center gap-3 px-4 py-3 bg-purple-50 border border-purple-200 rounded-xl">
          ...
          <div className="flex-1">
            <p className="text-xs text-purple-600 font-medium">행동 원인 분석 연동</p>
            <p className="text-sm text-gray-700">
              <strong>&apos;{functions.find(f => f.id === fromFbaFunction)?.label || fromFbaFunction}&apos;</strong> 행동 기능에 추천되는 전략을 필터링 중입니다.
            </p>
          </div>
          <Link href={`/${classCode}/fba`} className="text-xs text-purple-500 hover:text-purple-700 whitespace-nowrap">분석 화면으로</Link>
        </div>
      )}
...
export function InterventionsLibraryView() {
  return (
